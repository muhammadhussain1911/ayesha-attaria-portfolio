            ...
            {blogs.map((blog) => (
              <div key={blog.id} className="glass-card group hover:-translate-y-1 transition-all duration-300 p-6 flex items-center justify-between">
                ...
                <div className="flex gap-2 ml-4">
                  <Link href={`/admin/blogs/edit/${blog.slug}`} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                    <Edit2 className="w-5 h-5" />
                  </Link>
                  <button onClick={() => handleDelete(blog.id, blog.slug)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
